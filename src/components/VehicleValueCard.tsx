...
import { VehicleValueRequest, VehicleValueResponse, VehicleHistoryPoint, getVehicleValue, getVehicleHistory } from '../lib/supabase';
...
const VehicleValueCard = ({ initialRequest, onValueUpdate }: VehicleValueCardProps) => {
  ...
  const handleLookupValue = async () => {
    ...
    try {
      ...
      const valueResponse = await getVehicleValue(request);
      ...
      setError(err instanceof Error ? err.message : 'Failed to get vehicle value');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 space-y-6">
      <div className="border-b pb-4">
        <h3 className="text-xl font-semibold text-gray-900 mb-4">Vehicle Market Value</h3>
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              {trend && (
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500">vs. 30 days ago</p>
                </div>
              )}
            </div>
          </div>
          ...
          {history.length > 0 && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <h5 className="text-sm font-medium text-gray-900 mb-2">30-Day History</h5>
          ...
          <div className="text-xs text-gray-500">
            <p>Data provided by RapidAPI Vehicle Pricing • Last updated: {new Date(vehicleValue.timestamp).toLocaleString()}</p>
          </div>
        </div>
      )}
    </div>
